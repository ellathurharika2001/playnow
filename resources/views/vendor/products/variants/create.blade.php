...
<div class="admin-container">
    <div class="admin-header">
        <h1 class="admin-title">{{ isset($variant) ? 'Edit Variant' : 'Add New Variant' }}</h1>
    </div>
    ...
    <div class="admin-card">
        ...
        <form action="{{ isset($variant) ? route('vendor.products.variants.update', [$product, $variant]) : route('vendor.products.variants.store', $product) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @if (isset($variant))
                @method('PUT')
            @endif
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" name="sku" class="form-input"
                        value="{{ isset($variant) ? $variant->sku : old('sku') }}" required>
                    @error('sku')
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="stock_quantity" min="0" class="form-input"
                        value="{{ isset($variant) ? $variant->stock_quantity : old('stock_quantity') }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="weight" step="0.01" min="0" class="form-input"
                        value="{{ isset($variant) ? $variant->weight : old('weight') }}">
                    @error('weight')
                        <small class="text-error">{{ $message }}</small>
                    @enderror
                </div>
            ...
            <div class="form-group mt-3">
                <label class="form-label">Variant Image {{ !isset($variant) ? '*' : '' }}</label>
                @if (isset($variant) && $variant->variant_image)
                    <div class="mb-3">
                        <img src="{{ $variant->variant_image }}" alt="Variant" class="h-32 w-32 object-cover rounded">
                    </div>
                @endif
                <input type="file" name="variant_image" accept="image/*" class="form-input" {{ !isset($variant) ? 'required' : '' }}>
                            ...
                            @foreach ($attribute->values as $value)
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="attribute_values[]" value="{{ $value->attribute_value_id }}" 
                                        ...
                                        {{ isset($variant) && $variant->attributes->contains('attribute_value_id', $value->attribute_value_id) ? 'checked' : '' }}>
            ...
            <div class="form-group mt-3">
                <label class="inline-flex items-center">
                    <input type="checkbox" name="is_default" value="1" class="rounded border-gray-300"
                        {{ isset($variant) && $variant->is_default ? 'checked' : '' }}>
